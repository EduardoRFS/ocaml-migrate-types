...
module Asttypes = struct
  ...
  let copy_mutable_flag = Migrate_408_409.copy_mutable_flag

  let copy_private_flag = Migrate_408_409.copy_private_flag

  let copy_arg_label = Migrate_408_409.copy_arg_label

  let copy_virtual_flag = Migrate_408_409.copy_virtual_flag
end

module Parsetree = struct
  let copy_attributes = Migrate_parsetree.Migrate_408_409.copy_attributes
end
...
and copy_module_type : From.module_type -> To.module_type = function
  ...
  | Mty_functor (param_1, param_2, param_3) ->
      Mty_functor
        (param_1, Option.map copy_module_type param_2, copy_module_type param_3)
...
and copy_value_unbound_reason :
    From.value_unbound_reason -> To.value_unbound_reason = function
  | Val_unbound_instance_variable -> Val_unbound_instance_variable
  | Val_unbound_ghost_recursive -> Val_unbound_ghost_recursive
...
and copy_value_kind : From.value_kind -> To.value_kind = function
  ...
  | Val_unbound param_1 -> Val_unbound (copy_value_unbound_reason param_1)
